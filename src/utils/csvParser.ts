...
const FIELD_SYNONYMS: Record<keyof ColumnMapping, string[]> = {
  ...
  method: [
    ...
    "method (from project id) (from locations)",
    "method (from project id)",
    "method",
    "study method",
    "collection method",
    "equipment method",
  ...
  serviceTypeAddOns: [
    ...
    "service type add ons (from locations)",
  ...
  customId: [
    "custom id (from locations)",
    "custom id (from locations",
    "custom id",
    "customid",
    "custom_id",
    "custom id (from project id) (from locations)",
    "custom location id",
    "client custom id",
    "location custom id",
...
export function parseCsvData(csvText: string, customMapping?: Partial<ColumnMapping>): ParseResult {
  ...
  parsed.data.forEach((row, idx) => {
    ...
    const rawTechName = (row[mapping.technicianName] || "").trim();
    const techName = rawTechName ? formatTechnicianFullName(rawTechName) : "Unassigned Tech";
    ...
    const rawMethod = (
      ...
      row["Method (from Project ID) (from Locations)"] ||
      row["Method (from Project ID)"] ||
      row["Study Method"] ||
      ""
    ).trim();
    ...
    const rawScheduleNotes = (row[mapping.scheduleNotes] || "").trim();
    const scheduleNotes = rawScheduleNotes || "";

    const rawScheduleDetails = (row[mapping.scheduleDetails] || "").trim();
    const scheduleDetails = rawScheduleDetails || "";

    const rawCustomId = (
      row[mapping.customId] ||
      row["Custom ID (from Locations)"] ||
      row["Custom ID (from Locations"] ||
      row["Custom ID"] ||
      row["CustomID"] ||
      row["Custom_ID"] ||
      row["Custom ID (from Project ID) (from Locations)"] ||
      row["Custom Location ID"] ||
      ""
    ).trim();
    ...
    if (hasMultiActivityColumns) {
      ...
      if (setupParsed) {
        ...
        orders.push({
          ...
          customId: rawCustomId || undefined,
