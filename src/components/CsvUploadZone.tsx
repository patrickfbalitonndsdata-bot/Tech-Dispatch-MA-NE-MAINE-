...
export const CsvUploadZone: React.FC<CsvUploadZoneProps> = ({
  ...
  const isLoaded = (parseResult && parseResult.orders.length > 0) || !!currentFileName;
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-xl p-5 shadow-xs transition-all">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        ...
        <div className="flex-1 w-full">
          ...
          >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              <button
                type="button"
                className="w-full sm:w-auto text-xs font-semibold bg-zinc-900 hover:bg-zinc-800 text-white px-3.5 py-2 rounded-lg transition shadow-xs flex items-center justify-center gap-1.5"
              >
        ...
        {/* Right: Quick actions, mapping info & Clear button */}
        {isLoaded && (
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 shrink-0 w-full lg:w-auto">
            ...
            {onClear && (
              <button
                onClick={onClear}
                className="flex-1 sm:flex-initial flex items-center justify-center space-x-1.5 text-xs font-medium bg-white hover:bg-red-50 text-red-600 hover:text-red-700 px-3.5 py-2 rounded-lg border border-zinc-200 hover:border-red-200 transition shadow-xs cursor-pointer"
                title="Clear current CSV upload and reset generated emails"
              >
